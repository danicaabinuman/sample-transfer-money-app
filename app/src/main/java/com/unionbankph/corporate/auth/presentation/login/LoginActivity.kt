...
class LoginActivity :
    ...
    override fun onViewsBound() {
        ...
        if (!sharedPreferenceUtil.isLaunched().get()) {
            ...
            if (intent.getBooleanExtra(EXTRA_SPLASH_SCREEN, true)) {
            ...
            when (it) {
                ...
                is ShowLoginConnectivityError -> {
                    MaterialDialog(this).show {
                        ...
                        message(text = it.throwable.message.notEmpty())
                        positiveButton(
        ...
        if (isSME) {
            if (!viewModel.cdaoFeature.value.notNullable()) {
                ...
                constraintSet.connect(
                    ...
                    btn_initial_login.id, ConstraintSet.BOTTOM
                )
    ...
    private fun fingerprintViews(token: String) {
        ...
        if (token != "") {
            if (intent.getBooleanExtra(EXTRA_SPLASH_SCREEN, true) && !isShownInitialFingerprint) {
                runPostDelayed(
                    {
    ...
    private fun initClickTextViewLearnMore() {
        RxView.clicks(textViewLearnMore)
            ...
            .subscribe {
                navigator.navigate(
                    this, LearnMoreActivity::class.java, null, isClear = false, isAnimated = true,
                    transitionActivity = Navigator.TransitionActivity.TRANSITION_SLIDE_UP
                )
            }.addTo(disposables)
    }
