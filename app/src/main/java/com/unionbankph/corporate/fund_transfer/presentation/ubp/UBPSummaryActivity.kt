...
class UBPSummaryActivity :
    ...
    private val fundTransferVerify by lazyFast {
        JsonHelper.fromJson<FundTransferVerify>(
            intent.getStringExtra(EXTRA_FUND_TRANSFER_DTO)
        )
    }
    ...
    private lateinit var fundTransferUBPForm: FundTransferUBPForm
    ...
    private fun setupBinding() {
        viewModel.contextualClassFeatureToggle.subscribe {
            if (intent.getStringExtra(EXTRA_FUND_TRANSFER) != null
                ...
                setFundTransferDetails(fundTransferUBPForm, selectedAccount, it)
            }
        }.addTo(disposables)
    }
            ...
            .subscribe { granted ->
                if (granted) {
                    ...
                    MaterialDialog(this).show {
                        lifecycleOwner(this@UBPSummaryActivity)
