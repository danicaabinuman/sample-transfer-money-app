...
class SwiftViewModel @Inject constructor(
    ...
    private val _pesoNetFormState = MutableLiveData<SwiftState>()

    private val _account = MutableLiveData<Account>()

    val state: LiveData<SwiftState> get() = _pesoNetFormState
    ...
    fun getPurposes() {
        fundTransferGateway.getPurposes()
            ...
            .doOnSubscribe { _pesoNetFormState.value = ShowSwiftLoading }
            .doFinally { _pesoNetFormState.value = ShowSwiftDismissLoading }
            .subscribe(
                {
                    _pesoNetFormState.value = ShowSwiftGetPurposes(it)
                }, {
                    Timber.e(it, "getPurposes failed")
                    _pesoNetFormState.value = ShowSwiftError(it)
                })
            .addTo(disposables)
    }

    fun fundTransferSwift(fundTransferSwiftForm: FundTransferSwiftForm) {
        fundTransferGateway.fundTransferSwift(fundTransferSwiftForm)
            .subscribeOn(schedulerProvider.io())
            .observeOn(schedulerProvider.ui())
            .doOnSubscribe { _pesoNetFormState.value = ShowSwiftLoading }
            .doFinally { _pesoNetFormState.value = ShowSwiftDismissLoading }
            .subscribe(
                {
                    if (it.requestId != null) {
                        _pesoNetFormState.value = ShowSwiftSuccess(
                            Auth(
                        ...
                        _pesoNetFormState.value = ShowSwiftSkipOTPSuccess(it)
                    }
                }, {
                    Timber.e(it, "fundTransferSwift failed")
                    _pesoNetFormState.value = ShowSwiftError(it)
                }).addTo(disposables)
    }
...
sealed class SwiftState

object ShowSwiftLoading : SwiftState()

object ShowSwiftDismissLoading : SwiftState()

data class ShowSwiftSuccess(val auth: Auth) : SwiftState()

data class ShowSwiftSkipOTPSuccess(
    val fundTransferVerify: FundTransferVerify
) : SwiftState()

data class ShowSwiftGetPurposes(val list: MutableList<Purpose>) : SwiftState()

data class ShowSwiftError(val throwable: Throwable) : SwiftState()
