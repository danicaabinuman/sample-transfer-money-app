...
class PDDTSViewModel @Inject constructor(
    ...
    private val _pesoNetFormState = MutableLiveData<PDDTSState>()

    private val _account = MutableLiveData<Account>()

    val state: LiveData<PDDTSState> get() = _pesoNetFormState
    ...
    fun getPurposes() {
        fundTransferGateway.getPurposes()
            ...
            .doOnSubscribe { _pesoNetFormState.value = ShowPDDTSLoading }
            .doFinally { _pesoNetFormState.value = ShowPDDTSDismissLoading }
            .subscribe(
                {
                    _pesoNetFormState.value = ShowPDDTSGetPurposes(it)
                }, {
                    Timber.e(it, "getInstaPayPurposes failed")
                    _pesoNetFormState.value = ShowPDDTSError(it)
                })
            .addTo(disposables)
    }

    fun fundTransferPDDTS(fundTransferPesoNetForm: FundTransferPesoNetForm) {
            ...
            .subscribe(
                {
                    if (it.requestId != null) {
                        _pesoNetFormState.value = ShowPDDTSSuccess(
                            Auth(
                        ...
                        _pesoNetFormState.value = ShowPDDTSSkipOTPSuccess(it)
                    }
                }, {
                    Timber.e(it, "fundTransferPDDTS failed")
                    _pesoNetFormState.value = ShowPDDTSError(it)
                }).addTo(disposables)
    }
...
sealed class PDDTSState

object ShowPDDTSLoading : PDDTSState()

object ShowPDDTSDismissLoading : PDDTSState()

data class ShowPDDTSGetPurposes(val list: MutableList<Purpose>) : PDDTSState()

data class ShowPDDTSSuccess(val auth: Auth) : PDDTSState()

data class ShowPDDTSSkipOTPSuccess(
    val fundTransferVerify: FundTransferVerify
) : PDDTSState()

data class ShowPDDTSError(val throwable: Throwable) : PDDTSState()
