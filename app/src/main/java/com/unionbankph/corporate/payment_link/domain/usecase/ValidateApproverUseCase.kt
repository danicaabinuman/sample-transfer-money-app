...
import com.unionbankph.corporate.payment_link.domain.model.response.ValidateApproverResponse
import com.unionbankph.corporate.payment_link.domain.model.response.ValidateMerchantByOrganizationResponse
import io.reactivex.Single
import javax.inject.Inject

class ValidateApproverUseCase
@Inject constructor(
    threadExecutor: ThreadExecutor,
    postExecutionThread: PostExecutionThread,
    private val paymentLinkGateway: PaymentLinkGateway
) : SingleUseCase<ValidateApproverResponse, Unit>(
    threadExecutor,
    postExecutionThread
) {
    override fun buildUseCaseObservable(params: Unit?): Single<ValidateApproverResponse> {
        return paymentLinkGateway.validateApprover()
    }
